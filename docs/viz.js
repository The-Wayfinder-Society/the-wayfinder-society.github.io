...
function updateAllPlots() {
    ...
    songDataFilter = songDataFilter.filter(function(song) {
        ...
        if (topArtist) {
            // console.log("Filtering on top artist: " + topArtist.name);
            topArtistFilter = song['artists'].includes(topArtist['name']);
        };
        if (topTrack) {
            // console.log("Filtering on top tack: " + topTrack.name);
            topTrackFilter = song['name'] == topTrack['name'];
        }
        if (selectedTrack) {
            // console.log("Filtering on selected tack: " + selectedTrack.name);
            selectedTrackFilter = song['name'] == selectedTrack['name'];
        }
        if (selectedGenre) {
            // console.log("Filtering on selected genre: " + selectedGenre.name);
...
function setDefaults() {
    ...
    selectionContext['timeScale'] = 'short_term';
...
async function loadPage() {
    songDataGlobal = data.songDataGlobal;
    topArtistsGlobal = data.topArtistsGlobal;
    topTracksGlobal = data.topTracksGlobal;
    recentlyPlayedGlobal = data.recentlyPlayedGlobal;
    userProfileGlobal = data.userProfileGlobal;

    console.log(userProfileGlobal);

    genreDataGlobal = await d3.json("data_genres_average_features.json").then((d) => {return d}, (e) => {throw e});
    console.log(genreDataGlobal);
...
var data = {
    "songDataGlobal": songDataGlobal,
    "genreDataGlobal": genreDataGlobal,
    "topArtistsGlobal": topArtistsGlobal,
    "topTracksGlobal": topTracksGlobal,
    "recentlyPlayedGlobal": recentlyPlayedGlobal,
